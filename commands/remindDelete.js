  ...
  async execute(interaction) {
    ...
    if (deleteEntry == 0) {
      return interaction.reply({
        content:
          "**Warning:** No entries deleted, must enter a non-zero value.",
        ephemeral: true,
      });
    }
    ...
    fs.readFile("./remindLog.json", function (err, content) {
      ...
      if (userIter < 0) {
        return interaction.reply({
          content: "You have no active reminders, no entries deleted.",
          ephemeral: true,
        });
      }
      ...
      if (deleteEntry > userEntries.length) {
        return interaction.reply({
          content: "Entered entry number does not exist, no entries deleted.",
          ephemeral: true,
        });
      }
      ...
      return interaction.reply({
        content: `Deleted entry #${deleteEntry}, reminder successfully removed from scheduler.`,
        ephemeral: true,
      });
    });
  },
