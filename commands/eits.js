...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("eits")
    .setDescription(
      "Calculates the OP/DP of EITS results, must paste the results THEN immediately call the bot"
    )
  ...
  async execute(interaction) {
    ...
    const military = interaction.options.getInteger("military")
      ? interaction.options.getInteger("military")
      : 0;
    ...
    if (argArray.length < 15) {
      ...
      rawUnits = argArray
        .slice(argArray.length - 6, argArray.length)
        .toString()
    ...
    // City EITS reports
      ...
      rawUnits = argArray
        ...
        .split(",");
      GTs = argArray
        .slice(endOwner + 13, endOwner + 15)
        .toString()
        .replace("Guard,Towers:", "Guard Towers:")
        .split(":")[1];
      MTs = argArray
        .slice(endOwner + 11, endOwner + 13)
        .toString()
        .replace("Magic,Towers:", "Magic Towers:")
        .split(":")[1];
      walls = argArray
        .slice(endOwner + 20, endOwner + 21)
        .toString()
        .split(":")[1];
      extraArmies = argArray
        .slice(argArray.indexOf("city:") + 1, argArray.indexOf("city:") + 2)
        .toString()
        .split("(")[0];
      extraTroops = argArray.slice(
        argArray.indexOf("men)") - 1,
        argArray.indexOf("men)")
      );
