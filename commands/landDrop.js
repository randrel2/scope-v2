...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('dropcalc')
        .setDescription('Estimate time until resources acquired for Land Drop assuming no market interaction.')
        .addIntegerOption(option =>
            option.setName('buildings')
                .setDescription('# buildings to drop')
                .setRequired(true),
        ).addIntegerOption(option =>
            option.setName('goldcost')
                .setDescription('Gold cost per building')
                .setRequired(true),
        ).addIntegerOption(option =>
            option.setName('stonecost')
                .setDescription('Stone cost per building')
                .setRequired(true),
        ).addIntegerOption(option =>
            option.setName('woodcost')
                .setDescription('Wood cost per building')
                .setRequired(true),
        ).addIntegerOption(option =>
            option.setName('goldreserve')
                .setDescription('Current gold in reserve')
                .setRequired(true),
        ).addIntegerOption(option =>
            option.setName('stonereserve')
                .setDescription('Current stone in reserve')
                .setRequired(true),
        ).addIntegerOption(option =>
            option.setName('woodreserve')
                .setDescription('Current wood in reserve')
                .setRequired(true),
        ).addIntegerOption(option =>
            option.setName('goldprod')
                .setDescription('Takehome gold per hour')
                .setRequired(true),
        ).addIntegerOption(option =>
            option.setName('stoneprod')
                .setDescription('Stone production per hour')
                .setRequired(true),
        ).addIntegerOption(option =>
            option.setName('woodprod')
