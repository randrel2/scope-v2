...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("remind")
    .setDescription(
      "Bot will @ you with your message after X hours and Y minutes"
    )
  ...
  async execute(interaction) {
    ...
    if (minutes == 0 && hours == 0) {
      return interaction.reply({
        content:
          "**Warning:** No reminder set, must enter a non-zero value for hours and/or minutes.",
        ephemeral: true,
      });
    }
    ...
    fs.readFile("./remindLog.json", function (err, content) {
      ...
      fs.writeFile(
        "./remindLog.json",
        JSON.stringify(parseJson, null, 1),
        function (err) {
          if (err) throw err;
        }
      );
