...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('prep')
        .setDescription('Determines max prep based on city visual size and troop counts.')
    ...
    async execute(interaction) {
        ...
        const embedContent =
            `**${troops}** troops attacking city size **${citySize
            }**:\`\`\`00% Walls (${numeral(walls*0).format("00").toString().padStart(4,'0')}) | ${Math.ceil(
                results
            )} ticks\n25% Walls (${numeral(walls*0.25).format("00").toString().padStart(4,'0')}) | ${Math.ceil(
                results * 1.25
            )} ticks\n50% Walls (${numeral(walls*0.5).format("00").toString().padStart(4,'0')}) | ${Math.ceil(
                results * 1.5
            )} ticks\n75% Walls (${numeral(walls*0.75).format("00").toString().padStart(4,'0')}) | ${Math.ceil(
                results * 1.75
